export const GROK_MAIN_MODEL_SLOT = "10router";
...
const previousDefaultRegExp = /^# 10router-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# 10router-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via 10Router gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# 10router-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# 10router-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
