...
### ✨ 新增
- **更新日誌按介面語言載入**：Change Log 依目前語言顯示（中文 / 繁體 / English），隨包內建無延遲，未翻譯語言回退英文

### 🐛 修復
- **供應商排序修正**：有連線的供應商置頂，連線全部停用的（停用）沉底，從未新增的最後；OpenCode Free / MiMo Code Free 拓撲隱藏時排在已連線之後、停用之前
- **修復停用連線的計數**：停用的連線不再誤判為「已連線」
- **Change Log 彈窗**：外部連結新分頁開啟、彈窗與頂部選單接入多語言、切換語言即時重新整理
- **bai 等自訂模型不再被 JSON 目錄模式隱藏**

### 🔒 安全加固
- **出口代理開機自動恢復**：重啟後無需重存設定即恢復代理
